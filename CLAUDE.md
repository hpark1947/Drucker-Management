# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## Project Overview

Drucker Management는 피터 드러커(Peter Drucker)의 경영학 관련 콘텐츠를 정리하는 문서/지식 관리 프로젝트입니다.

## Structure

- `Prompts/` — 프롬프트 및 가이드 문서 보관 디렉토리

## GitHub 한글 깨짐 대응

GitHub에서 한글 파일이 유니코드 이스케이프 시퀀스(예: `\354\260\270\352\263\240`)로 표시되는 경우가 있다. 이는 GitHub의 트리 렌더링 캐시 문제로, 다음 절차로 해결한다:

1. 해당 파일에 공백 추가 등 사소한 변경을 가한 빈 커밋 또는 더미 커밋을 생성
2. push하여 GitHub 캐시를 갱신
3. GitHub 웹에서 한글이 정상 표시되는지 확인

이 문제가 발생하면 즉시 위 절차를 수행할 것.

## 작업 규칙

- `Prompts/`에 새 파일을 작성할 때, 반드시 `README.md`에 해당 문서 제목과 링크를 **먼저 추가한 뒤** 새 파일과 함께 **동시에 커밋·푸시**할 것 (루트 화면에서 즉시 확인 가능하도록)
- 파일 내용 작성이 끝나면 다시 커밋·푸시하여 최종본 반영

## 문체 규칙

콘텐츠 작성 시 반드시 `Prompts/guide.md`를 따른다:
- 대화체, 전문가→일반인 톤, 편안하고 다정한 분위기
- 전문 용어는 쉽게 풀이, 필요 시 예시를 들어 설명

## Git 커밋/푸시 규칙

- **MD 파일은 반드시 포함**: `.md` 파일은 프로젝트의 핵심 파일이므로, git commit 또는 push 시 변경된 `.md` 파일을 절대 누락하지 않는다.
  - 커밋 전 `git status`로 `.md` 파일 변경사항을 확인할 것
  - 스테이징 시 변경된 `.md` 파일이 빠지지 않았는지 반드시 점검할 것

## 참고

- 모든 문서는 한국어로 작성
